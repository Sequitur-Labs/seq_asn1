...
int seq_asn1_parse_der(SeqDerNode **destnode, uint8_t *buffer, size_t buffersize)
{
	...
	while (bufferoffset < buffersize && node) {

		res = fill_node(node, &nodesize, buffer, bufferoffset);
		if (res) {
			break;
		}

		bufferoffset += nodesize;
		if (bufferoffset > buffersize) {        //Poorly formed DER. Buffer too short.
			res = SEQ_AP_ERROR;
			break;
		}
